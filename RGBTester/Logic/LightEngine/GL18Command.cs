...
namespace RGBTester.Logic
{
    class GL18Command
    {
        ...
        public GL18Command(string portName)
        {
        ...
        private string SendCommand(byte[] packet)
        {
            if (!_serialPort.IsOpen)
            {
                try { _serialPort.Open(); }
                catch (Exception ex) { return "Error: 無法開啟序列埠。 " + ex.Message; }
            }
            ...
            {
                return "Error: 傳輸過程中發生錯誤。 " + ex.Message;
            }
        }

        public string SetLedDriverData(byte index, byte registerAddress, byte value)
        {
            ...
            // 3. 計算 Checksum (通常是對 Command, Index, Data 求和)
            packet[5] = CalculateChecksum(commandData); // CS

            // 4. 發送並回傳結果
            return SendCommand(packet);
        }

        public string GetLedDriverData(byte index, byte registerAddress)
        {
