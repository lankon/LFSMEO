...
{
    public class SubTaskOpticalTest_Debug : IBaseTask<SubTaskOpticalTest_Debug.WORK>
    {
        //將設定流程改成與示波器頁面控制方式相同,並且拿掉Reset
        
        public SubTaskOpticalTest_Debug(IBaseTaskDependence dependencies, 
                ...
                case WORK.GET_SPECTRUM:
                    {
                        if(CheckTimeOverMilSec(delay_time, 50)) //硬體轉換時間,需依實際狀況調整
                        {
                            ...
                            StreamWriter file = Tool.CreateFile($"\\Spectrum\\{DateTime.Now.ToString("yyyyMMddHH")}\\Spectrum_{DateTime.Now.ToString("yyyyMMddHHmmss")}", ".csv", false);
                            for(int i =0; i< SpectrumData.Count; i++)
                            {
                                double[] spc = SpectrumData[i];
