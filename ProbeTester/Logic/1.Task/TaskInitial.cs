        ...
        protected override void RunLoop(TASK_STATUS task_command)
        {
            ...
            switch (State)
            {
                case WORK.INITIAL:
                    {
                        Preset();
                        Transition(WORK.GO_HOME_XYZA);
                    }
                    break;

                case WORK.GO_HOME_XYZA:
                    {
                        ...
                        Transition(WORK.WAIT_GO_HOME_XYZA);
                    }
                    break;
                case WORK.WAIT_GO_HOME_XYZA:
                    {
                        if(Deps.DML.Get_Home_Complete(Axis.AxisX) && Deps.DML.Get_Home_Complete(Axis.AxisY) &&
                           Deps.DML.Get_Home_Complete(Axis.AxisZ) && Deps.DML.Get_Home_Complete(Axis.AxisA))
                        {
